...
# data
parser.add_argument('--dataset_name', type=str, default='weather')
if syst == 'Windows':
    parser.add_argument('--train_data_paths', type=str,
                        default='data/datasets2/train/weather.csv,data/datasets2/train/moran.csv')
    parser.add_argument('--valid_data_paths', type=str,
                        default='data/datasets2/val/weather.csv,data/datasets2/val/moran.csv')
    parser.add_argument('--test_data_paths', type=str,
                        default='data/datasets2/test/weather.csv,data/datasets2/test/moran.csv')
else:
    parser.add_argument('--train_data_paths', type=str, default='datasets/train/weather.csv,datasets/train/moran.csv')
    parser.add_argument('--valid_data_paths', type=str, default='datasets/val/weather.csv,datasets/val/moran.csv')
    parser.add_argument('--test_data_paths', type=str, default='datasets/test/weather.csv,datasets/test/moran.csv')
...
parser.add_argument('--gen_frm_dir', type=str, default='predict')
parser.add_argument('--input_length', type=int, default=2)
parser.add_argument('--total_length', type=int, default=4)
parser.add_argument('--img_width', type=int, default=86)  #changed this
parser.add_argument('--img_channel', type=int, default=1)

# model
parser.add_argument('--model_name', type=str, default='dconvlstm_sac')
...
parser.add_argument('--sampling_stop_iter', type=int, default=5000)
...
parser.add_argument('--num_save_samples', type=int, default=200)
...
parser.add_argument('--MaxValue', type=float, default='1.0')   #First change
parser.add_argument('--MinValue', type=float, default='0.0')   #Second change
parser.add_argument('--gx_out_file', type=str, default='gx-out')
parser.add_argument('--x_out_file', type=str, default='x-out')
...
def reserve_schedule_sampling_exp(itr):
    ...
    real_input_flag = np.reshape(real_input_flag,
                                 (args.batch_size,
                                  ...
                                  #args.total_length -2, changed this
                                  args.img_width // args.patch_size,
                                  args.img_width // args.patch_size,
                                  args.patch_size * args.patch_size * args.img_channel))
...
def train_wrapper(model):
    ...
    train_input_handle, valid_input_handle, test_input_handle = datasets_factory.data_provider(
        ...
        args.img_width, input_length = args.input_length,
    ...
    for itr in range(1, args.max_iterations + 1):
        ...
        for i in trange(int(train_input_handle.total() / args.batch_size)):
            ...
            #print(f'ims shape before preprocess: {ims.shape}')
            ims = preprocess.reshape_patch(ims, args.patch_size, args.model_name)
            #print(f'ims shape after preprocess: {ims.shape}')
...
def test_wrapper(model):
    ...
    test_input_handle = datasets_factory.data_provider(
        args.dataset_name, args.train_data_paths, args.valid_data_paths, args.test_data_paths, args.batch_size, args.img_width, input_length = args.input_length,
        seq_length=args.total_length, injection_action=args.injection_action, is_training=False)
    #trainer.test(model, test_input_handle, args, 'test_result')
    trainer.test(model, test_input_handle, args, 5)
