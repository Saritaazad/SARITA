...
class InputHandle:
    ...
    def begin(self, do_shuffle=False):
        ...
        print(f'indices are:{self.indices}')
        if do_shuffle:
    ...
    def input_batch(self):
        ...
        for i in range(self.current_batch_size):
            ...
            # Dynamically adjust length based on data_slice shape
            slice_length = data_slice.shape[0]
            input_batch[i, :slice_length, :, :, :] = data_slice
...
if __name__=="__main__":
    # Test the InputHandle class
    input_param = {
        'paths': ["../../data/moving-mnist-example/moving-mnist-test.npz"],
        'name': 'mock_dataset',
        'minibatch_size': 4,
    ...
    # Shuffle data and begin batching
    ...
    while not input_handle.no_batch_left():
        ...
        print(f"Input shape: {batch[:, :input_handle.current_input_length].shape}")
        print(f"Output shape: {batch[:, input_handle.current_input_length:].shape}")
        input_handle.next()
        batch_count += 1

    print("Testing completed.")
    # Initialize InputHandle instance
    # input_handle = InputHandle(input_param)
    #
    # # Shuffle data and begin batching
    # input_handle.begin(do_shuffle=True)
