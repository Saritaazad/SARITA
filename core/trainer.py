...
def valid(model, valid_input_handle, configs, itr):
    ...
    real_input_flag = np.zeros(
        (configs.batch_size,
         configs.total_length - mask_input- 1, #modified this to make it work
    ...
    while not valid_input_handle.no_batch_left():
        ...
        test_dat = preprocess.reshape_patch(test_ims, configs.patch_size, configs.model_name)
        ...
        for i in range(output_length):
            ...
            #print(f'x shape: {x.shape}, gx shape: {gx.shape}')
            org.append(x)
            pre.append(gx)
            data_range = configs.MaxValue - configs.MinValue
            for b in range(configs.batch_size):
                score, _ = SSIM(x[b], gx[b], full=True, multichannel=True, data_range=data_range)
    ...
    #print(f'pre shape: {pre}, org shape: {org}')
...
def test(model, test_input_handle, configs, itr):
    ...
    while (test_input_handle.no_batch_left() == False):
        ...
        for i in range(output_length):
            ...
            for b in range(configs.batch_size):
                ...
                append_row(np.array(gx[b]).reshape(configs.img_width*configs.img_width))
        print("test_batch_id:", batch_id)
    ...
    mae_li = np.array(mae_li).reshape(configs.img_width, configs.img_width)
    rmse_li = np.array(rmse_li).reshape(configs.img_width, configs.img_width)
    r2_li = np.array(r2_li).reshape(configs.img_width, configs.img_width)
    mse_li = np.array(mse_li).reshape(configs.img_width, configs.img_width)
    relevant_li = np.array(relevant_li).reshape(configs.img_width, configs.img_width)
    from PIL import Image
