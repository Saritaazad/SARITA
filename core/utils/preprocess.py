...
def reshape_patch(img_tensor, patch_size, model_name):
    assert 5 == img_tensor.ndim
    #img_tensor = np.transpose(img_tensor, [0, 1, 4, 2, 3])
    if model_name in ['convlstm', 'dconvlstm']:
        img_tensor = img_tensor[:,:,:,:,0]
        img_tensor = np.reshape(img_tensor, (1, 12, 86, 86, 1))
